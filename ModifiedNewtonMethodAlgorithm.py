...
'''Modified Newton Method Algorithm'''
...
def ModifiedNewton(sympy_function, max_iter, start, step_size = 1, epsilon = 10**-2, zeta = 0.1):
    i = 0
    ...
    while i < max_iter:
        ...
        if norm < epsilon:
            ...
            hessian_inv = LA.inv(hessian(sympy_function)+np.multiply(zeta,np.identity(2)) )
            p = -np.dot(grad, hessian_inv)
            x_values[i+1] = x_values[i] + step_size*p
            norm_values.append(norm)
        i+=1
    print("No. of steps Modified Newton method takes to converge: ", len(norm_values))
    print('')
    return(x_values, norm_values)

x_MNM, norm_values = ModifiedNewton(sympy_function=f, max_iter=1000, start =[1,1] , step_size = 1, epsilon = 10**-2)
print(x_MNM)

print('')
print("contour path through Modified Newton's method")
Contour_path = contour_travel(x_MNM, f)
